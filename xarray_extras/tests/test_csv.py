...
def assert_to_csv(
    x, chunks, nogil, dtype, open_func=open, float_format="%f", ext="csv", **kwargs
):
    x = x.astype(dtype)
    if chunks:
        x = x.chunk(chunks)
    with tempfile.TemporaryDirectory() as tmp:
        ...
        f = to_csv(
            x, tmp + "/2." + ext, nogil=nogil, float_format=float_format, **kwargs
        )
        dask.compute(f)
...
@pytest.mark.parametrize("x", [0, np.nan, 1.000000000000001, 1.7901234406790122e308])
@pytest.mark.parametrize("index,coord", [(False, ""), (True, "a"), (True, "a" * 1000)])
...
def test_mode(chunks, nogil):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        f = to_csv(x, tmp + "/1.csv", mode="a", nogil=nogil, header=False, index=False)
        dask.compute(f)
        f = to_csv(y, tmp + "/1.csv", mode="a", nogil=nogil, header=False, index=False)
        dask.compute(f)
        with open(tmp + "/1.csv") as fh:
            assert fh.read() == "1\n2\n3\n4\n"

        f = to_csv(y, tmp + "/1.csv", mode="w", nogil=nogil, header=False, index=False)
        dask.compute(f)
        with open(tmp + "/1.csv") as fh:
            assert fh.read() == "3\n4\n"
