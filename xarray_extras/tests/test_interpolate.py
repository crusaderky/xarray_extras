...
def test_0d(k, expect):
    ...
    """
    y = DataArray(
        [[10, 20, 30, 40, 50, 60], [11, 28, 39, 55, 15, -2], [np.nan, 2, 3, 4, 5, 6]],
        dims=["y", "x"],
        coords={"x": [1, 2, 3, 4, 5, 6], "y": ["y1", "y2", "y3"]},
    )
    tck = splrep(y, "x", k)
    expect = DataArray(
        expect, dims=["y"], coords={"x": 4.5, "y": ["y1", "y2", "y3"]}
    ).astype(float)
...
@pytest.mark.parametrize(
    ...
    [
        # list
        ([3.5, 4.5], DataArray([35.0, 45.0], dims=["x"], coords={"x": [3.5, 4.5]})),
        # tuple
        ((3.5, 4.5), DataArray([35.0, 45.0], dims=["x"], coords={"x": [3.5, 4.5]})),
        # np.array
        (
            np.array([3.5, 4.5]),
            DataArray([35.0, 45.0], dims=["x"], coords={"x": [3.5, 4.5]}),
        ),
        # da.Array
        (
            da.from_array(np.array([3.5, 4.5]), chunks=1),
            DataArray([35.0, 45.0], dims=["x"], coords={"x": [3.5, 4.5]}).chunk(1),
        ),
        # DataArray, same dim as y, no coord
        (DataArray([3.5, 4.5], dims=["x"]), DataArray([35.0, 45.0], dims=["x"])),
        # DataArray, same dim as y, with coord
        (
            DataArray([3.5, 4.5], dims=["x"], coords={"x": [100, 200]}),
            DataArray([35.0, 45.0], dims=["x"], coords={"x": [100, 200]}),
        ),
        # DataArray, different dim as y
        (DataArray([3.5, 4.5], dims=["t"]), DataArray([35.0, 45.0], dims=["t"])),
    ],
)
def test_1d(x_new, expect):
    ...
    """
    y = DataArray(
        [10, 20, 30, 40, 50, 60], dims=["x"], coords={"x": [1, 2, 3, 4, 5, 6]}
    )
    tck = splrep(y, "x", k=1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, False, {"y": (1, 1), "x": (6,)}, ((2,), (2,), (1, 1))),
        (True, True, {"y": (1, 1), "x": (6,)}, ((1, 1), (1, 1), (1, 1))),
    ],
)
def test_nd(chunk_y, chunk_x_new, expect_chunks_tck, expect_chunks_y_new):
    ...
    y = DataArray(
        ...
        dims=["y", "x"],
        coords={"x": [1, 2, 3, 4, 5, 6], "y": ["y1", "y2"]},
    )
    x_new = DataArray(
        [[3.5, 4.5], [1.5, 5.5]],
        dims=["w", "z"],
        coords={"w": [100, 200], "z": ["foo", "bar"]},
    )
    expect_tck = Dataset(
        data_vars={
...
def test_transpose(transpose, contiguous):
    ...
    y = DataArray(
        [[10, 20], [30, 40], [50, 60]],
        dims=["y", "x"],
        coords={"x": [1, 2], "y": ["y1", "y2", "y3"]},
    )
    expect = DataArray(
        [[15.0, 35.0, 55.0], [10.0, 30.0, 50.0]],
        dims=["x", "y"],
        coords={"x": [1.5, 1.0], "y": ["y1", "y2", "y3"]},
    )
...
@pytest.mark.parametrize(
    "x_new_dtype", [np.uint32, np.uint64, np.int32, np.int64, np.float32]
)
@pytest.mark.parametrize(
    "x_dtype", [np.uint32, np.uint64, np.int32, np.int64, np.float32]
)
def test_nonfloat(x_dtype, x_new_dtype):
    """Test numeric x that isn't float64"""
    x = np.array([0, 100])

    y = DataArray((x * 3).astype(x_dtype), dims=["x"], coords={"x": x.astype(x_dtype)})
...
def test_dim_collision():
    ...
    y = DataArray(
        [[10, 20], [11, 28]], dims=["y", "x"], coords={"x": [1, 2], "y": ["y1", "y2"]}
    )
